...
export class NftMarketplace extends ethereum.SmartContract {
  ...
  ERC721Count(): BigInt {
    let result = super.call("ERC721Count", "ERC721Count():(uint256)", []);

    return result[0].toBigInt();
  }

  try_ERC721Count(): ethereum.CallResult<BigInt> {
    let result = super.tryCall("ERC721Count", "ERC721Count():(uint256)", []);
    if (result.reverted) {
      return new ethereum.CallResult();
    }
    let value = result.value;
